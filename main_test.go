...
func TestRun(t *testing.T) {
	testCases := map[string]struct {
		...
		"ok: rotate group access token then update project CICD var": {
			cmdArgs: []string{"--config", t_helper.FixturePath("configs", "cmd_test_config.yml"), "--force", "--strict"},
			mockGitlabResp: func(w http.ResponseWriter, r *http.Request) {
				...
				if path == `/api/v4/groups//some/group/path/access_tokens` && method == http.MethodGet {
					_, _ = w.Write(t_helper.ReadFixture("api_responses/group_access_tokens.json"))
				} else if path == `/api/v4/groups//some/group/path/access_tokens/42/rotate` && method == http.MethodPost {
					_, _ = w.Write(t_helper.ReadFixture("api_responses/group_access_token_rotate.json"))
				} else if path == `/api/v4/projects//some/repo/path/variables/THIS_IS_VAR` && method == http.MethodPut {
					_, _ = w.Write(t_helper.ReadFixture("api_responses/project_cicd_var.json"))
				}
			},
		},
		"ok: dry run enabled": {
			cmdArgs: []string{"--config", t_helper.FixturePath("configs", "cmd_test_config.yml"), "--dry-run", "--force"},
			mockGitlabResp: func(w http.ResponseWriter, r *http.Request) {
				w.WriteHeader(http.StatusOK)
				method := r.Method
				path := r.URL.Path
		...
		"err: required config is not set": {
			cmdArgs:        []string{"--config", t_helper.FixturePath("configs", "cmd_test_config.yml"), "--debug"},
			expectedErrMsg: "empty host",
		},
		"err: while read broken configuration fiel": {
			cmdArgs:        []string{"--config", t_helper.FixturePath("configs", "broken_config.yml")},
